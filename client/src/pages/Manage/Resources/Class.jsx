...
const ClassModal = ({ title, showModal, setShowModal, setData }) => {
  ...
  const handleDone = () => {
    console.log(semester + division + department);
    if (
      division.trim() == "" ||
      department.trim() == "" ||
      semester.trim() == ""
          ...
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="flex flex-col space-y-4">
              ...
              <input
                ...
                placeholder="Department I.T or C.E"
              />
            </div>
          </div>
...
export default function Class() {
  ...
  useEffect(() => {
    AXIOS.get("/v1/manageresource/class").then((res) => {
  ...
  const handleDelete = async () => {
    await AXIOS
      .patch("/v1/manageresource/class", checkedKeys)
      .then((e) => {
  ...
  const handleSearch = () => {
    ...
    const filteredData = data.filter((item) => {
      return (
        item.semester.toLowerCase().includes(searchInput.toLowerCase()) ||
        item.division.toLowerCase().includes(searchInput.toLowerCase()) ||
